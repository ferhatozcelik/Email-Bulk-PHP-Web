                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        if (isset($_GET['id'])) {
                            ...
                            $query = "SELECT * FROM bulk_email_list WHERE id='$email_id' ";
                            $query_run = mysqli_query($conn, $query);

                            if (mysqli_num_rows($query_run) > 0) {
                                $bulk_email_list = mysqli_fetch_array($query_run);
                        ?>
                                <form action="db-manager.php" method="POST">
                                    <input type="hidden" name="email_id" value="<?= $bulk_email_list['id']; ?>">

                                    <div class="mb-3">
                                        <label>Name</label>
                                        <input type="text" name="name" value="<?= $bulk_email_list['name']; ?>" class="form-control">
                                    </div>
                                    <div class="mb-3">
                                        <label>Email</label>
                                        <input type="email" name="email" value="<?= $bulk_email_list['email']; ?>" class="form-control">
                                    </div>
                                    <div class="mb-3">
                                        <label>Group</label>
                                        <input type="text" name="group" value="<?= $bulk_email_list['group']; ?>" class="form-control">
                                    </div>
                                    <div class="mb-3">
